...
curl --create-dirs --continue-at - "${NO_AF_MODEL_BUCKET}/model.ckpt.{data-00000-of-00001,index,example_info.json,meta}" --output "tutorial/existing_ckpts/DeepVariant/v${BIN_VERSION_DV}_withIS_noAF/model.ckpt.#1"
...
# Use the same file naming convention and enable restarting of any interrupted downloads
curl --create-dirs --continue-at - "${AF_MODEL_BUCKET}/model.ckpt.{data-00000-of-00001,index,meta,example_info.json}" --output "tutorial/existing_ckpts/DeepVariant/v${BIN_VERSION_DV}_withIS_withAF/wgs_af.model.ckpt.#1"
