#!/bin/bash
# scripts/setup/tutorial_human.sh
echo -e "=== scripts/setup/tutorial_human.sh > start $(date)"
...
# downloading a complete trio. We'll also need to download the reference PopVCF 
# used to build the humanWGS_AF model.
...
# A preprint describing these trios is at https://doi.org/10.1101/2020.07.24.212712.  
# The paper(s) above can be cited for use of the benchmark, and please 
# cite http://www.nature.com/articles/sdata201625 (doi:10.1038/sdata.2016.25) when using the 
# corresponding sequencing data.

#-------------------------------------------------------------------#

# Create a place to store data for tutorial
DIR_NAME="./tutorial/data/human"
install --directory --verbose $DIR_NAME
cd $DIR_NAME
...
if [ ! -f "./allele_freq/PopVCF.merge.list" ]; then 
    for i in {{1..22},X,Y}
    do
        echo "./$DIR_NAME/allele_freq/cohort-chr$i.release_missing2ref.no_calls.vcf.gz" >> ./allele_freq/PopVCF.merge.list
    done
fi

if [ ! -f "./allele_freq/concat_PopVCFs.sh" ]; then 
echo -e "source ./scripts/setup/modules.sh
bcftools concat --file-list $DIR_NAME/allele_freq/PopVCF.merge.list -Oz -o $DIR_NAME/allele_freq/cohort.release_missing2ref.no_calls.vcf.gz
bcftools index $DIR_NAME/allele_freq/cohort.release_missing2ref.no_calls.vcf.gz" > ./allele_freq/concat_PopVCFs.sh
fi
...
download () {
    ...
    if [ ! -f "./bam/${trio_name}.download" ]; then 
        ...
        # Next, write the correct MD5 + filepath to a new file, and
        # print the command for checking the MD5 sum.
        echo "DIR_NAME=${DIR_NAME}" > "./bam/${trio_name}.download" 
        awk -F '\t' 'NR!=1  { 
            cmd = "basename " $1
            ...
            index(filename, "GRCh38") {
                ...
                print "curl -o $DIR_NAME/bam/"filename" -C - "$1" --keepalive-time 300"
                ...
                filename ~ /HG002.GRCh38/ {
                    ...
                    close(cmd)
                    split(result, output, " ")
                    print "echo "output[1]"\t$DIR_NAME/bam/"filename" > $DIR_NAME/bam/"filename".md5"
                }

                filename !~ /HG002/ && filename ~ /GRCh38/ {
                    print "echo "$2"\t$DIR_NAME/bam/"filename" > $DIR_NAME/bam/"filename".md5"
                }

            index(filename, "GRCh38") {
                print "echo $(date \"+%Y-%m-%d %H:%M:%S\") INFO: checking ["filename"] for corruption..."
                print "md5sum -c $DIR_NAME/bam/"filename".md5"
            ...
            index(secondfile, "GRCh38") {
                ...
                print "curl -o $DIR_NAME/bam/"secondfile" -C - "$3
                ...
                secondfile ~ /HG002.GRCh38/ {
                    ...
                    close(cmd)
                    split(result, output, " ")
                    print "echo "output[1]"\t$DIR_NAME/bam/"secondfile" > $DIR_NAME/bam/"secondfile".md5"
                }

                secondfile !~ /HG002/ && secondfile ~ /GRCh38/ {
                    print "echo "$4"\t$DIR_NAME/bam/"secondfile" > $DIR_NAME/bam/"secondfile".md5"
                }

            index(secondfile, "GRCh38") {
                print "echo $(date \"+%Y-%m-%d %H:%M:%S\") INFO: checking ["secondfile"] for corruption..."
                print "md5sum -c $DIR_NAME/bam/"secondfile".md5"
                print "echo $(date \"+%Y-%m-%d %H:%M:%S\") INFO: checking ["secondfile"] for corruption... done"

                print "echo ===================================================="
            }' "./bam/${file_input}"  >> "./bam/${trio_name}.download"
    else
        echo "$(date '+%Y-%m-%d %H:%M:%S') INFO: file found | '$$DIR_NAME/bam/${trio_name}.download'"
    fi
    }
...
calc_cov () {
    ...
    if [ ! -f "./bam/${trio_name}.run" ]; then
        echo "DIR_NAME=${DIR_NAME}" > ./bam/${trio_name}.run
        ...
        awk -F '\t' 'NR!=1 { 
            ...
            index(filename, "GRCh38") {
                ...
                print "if [ ! -f $DIR_NAME/bam/"label".coverage.out ]; then"
                print "    echo \"$(date \"+%Y-%m-%d %H:%M:%S\")  INFO: Calculating Coverage for ["label"] now...\"" 
                print "    samtools coverage $DIR_NAME/bam/"filename" --output $DIR_NAME/bam/"label".coverage.out"
                print "    echo \"$(date \"+%Y-%m-%d %H:%M:%S\")  INFO: Calculating Coverage for ["label"] now... done\""
                print "else"
                print "    echo \"$(date \"+%Y-%m-%d %H:%M:%S\")  INFO: bash file found | $DIR_NAME/bam/"label".coverage.out\""
                print "fi"
                print "echo ----------------------------------------------------"
                split(label, output, ".")
                print "if [ ! -f $DIR_NAME/bam/"output[1]".avg_coverage.out ]; then"
                print "    echo \"$(date \"+%Y-%m-%d %H:%M:%S\")  INFO: Calculating Average Coverage for ["output[1]"] now...\"" 
                print "    awk '\''{ total += $6; count++ } END { print \""label" AVERAGE COVERAGE = \" total/count }'\'' $DIR_NAME/bam/"label".coverage.out > $DIR_NAME/bam/"output[1]".avg_coverage.out"
                print "    echo \"$(date \"+%Y-%m-%d %H:%M:%S\")  INFO: Calculating Average Coverage for ["output[1]"] now... done\""
                print "else"
                print "    echo \"$(date \"+%Y-%m-%d %H:%M:%S\")  INFO: output file found | $DIR_NAME/bam/"output[1]".avg_coverage.out\""
                print "fi" 
        ...
        echo "$(date '+%Y-%m-%d %H:%M:%S') INFO: file found | '$DIR_NAME/bam/${trio_name}.run'"
    fi
    }
...
echo -e "=== scripts/setup/tutorial_human.sh > end $(date)"
